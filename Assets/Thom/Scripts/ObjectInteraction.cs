...
public class ObjectInteraction : Singleton<ObjectInteraction> {
    ...
    private float minimumScale = 0.003f;
    ...
    private void Update()
    {
        if (moving)
        {
            objectInFocus.transform.position = new Vector3(CursorManagerThom.Instance.Cursor.transform.position.x, CursorManagerThom.Instance.Cursor.transform.position.y, CursorManagerThom.Instance.Cursor.transform.position.z);
        }    
    }
        ...
        if (objectInFocus.tag == "spawnobject" || objectInFocus.tag == "newspawnobject")
        {
            ...
            if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, 20.0f, layerMask))
            {
                ...
                {   
                    ...
                    CursorManagerThom.Instance.SetCursorIcon(UIManager.Instance.currentMode);
                }
                else
                {
